...
- 文字は **Penn Station**(時刻 72px、和暦・西暦 39px、月日・曜日 31px)で描画しています。
...
| パス | 内容 |
| --- | --- |
| `src/c/main.c` | 文字盤本体 (レイアウト・和暦計算) |
| `resources/fonts/PennStation.ttf` | 文字用フォント (ビルド時に数字・英大文字のみに絞って収録) |
| `package.json` | プロジェクト定義 (対象プラットフォーム: emery、フォントリソース) |
| `wscript` | ビルドスクリプト |

Penn Station フォントの利用条件は配布元によって異なる場合があります。このリポジトリや文字盤を公開・配布する場合は、入手元のライセンス条件を確認してください。
